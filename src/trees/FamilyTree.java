...
public class FamilyTree {
	...
	private static class TreeNode {
		...
		ArrayList<TreeNode> collectAncestorsToList() {
			...
			// HINT: going up
			// the nodes of a tree is like traversing a linked list. If that
			// isn’t clear,
			// draw a tree, mark any leaf node, and then mark its ancestors in
			// order from
			// recent to ancient. Expect a question about this on the final
			// exam.
			TreeNode tempIterator = this;
			// Instead of "next" we have "parent" to access the next element
			while (tempIterator.parent != null) {
				ancestors.add(tempIterator.parent);
				tempIterator = tempIterator.parent;
			}
	...
	public FamilyTree() throws IOException, TreeException {
		...
		FileReader fr = new FileReader(
				"/home/surikoya/eclipse-workspace/lab12/src/BagginsFamilyTree.txt");
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			throw new TreeException("Bad colonIndex");
...
=======
	...
	// depth of the root is 0. The
>>>>>>> b266c1d (Saving changes)
...
=======
			TreeNode ancestor = tree.getMostRecentCommonAncestor("Bilbo",
					"Frodo");
			System.out.println(
					"Most recent common ancestor of Bilbo and Frodo is "
							+ ancestor.getName());
>>>>>>> b266c1d (Saving changes)
		} catch (IOException x) {
